...
/// Basic blocks are discussed in the [LLVM 12 docs on Functions](https://releases.llvm.org/13.0.0/docs/LangRef.html#functionstructure)
...
fn term_needs_name(term: LLVMValueRef) -> bool {
    ...
    match unsafe { LLVMGetInstructionOpcode(term) } {
        LLVMOpcode::LLVMInvoke => {
            let kind =
                unsafe { LLVMGetTypeKind(LLVMGetReturnType(LLVMGetCalledFunctionType(term))) };
            kind != LLVMVoidTypeKind
        }
