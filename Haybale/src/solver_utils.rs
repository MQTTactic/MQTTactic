...
pub fn sat(btor: &Btor) -> Result<bool> {
    match btor.sat() {
        ...
        SolverResult::Unknown => {
            println!("`SolverError`: the solver returned this error while evaluating a query: The query was interrupted, timed out, or otherwise failed");
            return Ok(true);
        }
    ...
    B: BV,
{
    ...
    for constraint in constraints {
        constraint.assert()?;
        //println!("DEBUG: {:?}", constraint.clone());
    }
    let retval = sat(btor);
    // println!(
    //     "DEBUG: {}\n----------------------------\n{:?}",
    //     btor.print_constraints(),
    //     retval.clone()
    // );
    btor.pop(1);
    retval
