...
pub fn symex_function<'p, B: Backend>(
    ...
    let mut keyBBs: Vec<String> = Vec::new();
    let mut keyFuncs: Vec<String> = Vec::new();

    // 读取keyBBs，destBB
    let mut fileName =
        String::from("/home/szx/Documents/Experiments/llvm-pass/HelloWorld/OUTPUT/PATHS/");
    fileName += &String::from(funcname);
    fileName += &String::from("/Type-0/");
    fileName += &String::from("path.0");
    ...
    for line in fin.lines() {
        ...
        let tokens: Vec<&str> = new_line.split(":").collect();

        keyBBs.push(String::from(new_line));
    }

    /*

    destBB.push(String::from("db__message_release_incoming"));
    destBB.push(String::from("send__pubcomp"));
    destBB.push(String::from("sub__messages_queue"));
    */
    let destBB = String::from("0");
...
// syncxxx-2-9
pub fn symex_function2<'p, B: Backend>(
    funcname: String,
    project: &'p Project,
    config: Config<'p, B>,
    params: Option<Vec<ParameterVal>>,
) -> Result<ExecutionManager<'p, B>> {
    let tokens: Vec<&str> = funcname.split(":").collect();
    let funcn = tokens[0];
    let fileName = tokens[1];
    let destBB = String::from("0");
    debug!("Symexing function {}", funcn);
    let (func, module) = project
        .get_func_by_name(funcn)
        .unwrap_or_else(|| panic!("Failed to find function named {:?}", funcn));
    let start_loc = Location {
        module,
        func,
        bb: func
            ...
            .get(0)
            .expect("Failed to get entry basic block"),
        //bb: func.get_bb_by_name(&Name::from(originBB)).unwrap(),
        instr: BBInstrIndex::Instr(0),
        source_loc: None, // this will be updated once we get there and begin symex of the instruction
    };
    // syncxxx-4-17 输出所有bb
    // for bb in func.basic_blocks.iter() {
    //     for (instnum, inst) in bb
    //         .instrs
    //         .iter()
    //         .enumerate()
    //     {
    ...
    let fileN = String::from(fileName);
    ...
    for line in fin.lines() {
        let new_line = (&line.unwrap()).clone();
        let tokens: Vec<&str> = new_line.split(":").collect();
        if !(keyFuncs.contains(&(String::from(tokens[0])))) {
            keyFuncs.push(String::from(tokens[0]));
        }
        keyBBs.push(String::from(new_line));
    }
...
pub struct ExecutionManager<'p, B: Backend> {
    ...
    squash_unsats: bool,
    // syncxxx-1-11 需要被遍历到的BasicBlocks
    keyBBs: Vec<String>,
    destBB: String,
    keyFuncs: Vec<String>,
}
    ...
    B: 'p,
{
    ...
    fn symex_from_cur_loc_through_end_of_function(&mut self) -> Result<Option<ReturnValue<B::BV>>> {
        //println!("DEBUG: {}", self.state.solver.print_constraints());
        println!(
            "DEBUG: {}:{:?}",
            self.state.cur_loc.func.name, self.state.cur_loc.bb.name
        );
        //syncxxx-1-25
        // let bbfunc = self.state.cur_loc.func.name.to_string();
        // let bbName = self.state.cur_loc.bb.name.to_string();
        // let s = bbfunc + &String::from("+") + &bbName;

        // if s == self.destBB {
        //     println!("now:{}; dest:{}", s, self.destBB);
        //     return Err(Error::PathComplete);
        // }
            ...
            .skip(insts_to_skip)
        {
            ...
            // let wp = BV::from_hex_str(self.state.solver.clone(), "10043f68", 64 as u32);
            // println!("aaaaaDEBUG {}, {:?}",self.state.cur_loc.instr, self.state.read(&wp, 64 as u32));
        ...
        // println!("DEBUG:{}",self.state.cur_loc.bb.term);
        ...
        match term {
            ...
            let retV = self.symex_return(ret).map(Some);
            self.state.varmap.clear_version_num(self.state.cur_loc.func.name.clone());
            // syncxxx-3-8  
            if self.state.cur_loc.func.name == self.func.name {
                return Err(Error::PathComplete);
            //     let one = self.state.one(32);
            //     let retC = retV.clone();
            //    let retval =  match retC.transpose() {
            //         Some(Ok(ReturnValue::Return(r))) => r,
            //         Some(Ok(ReturnValue::ReturnVoid)) => one,
            //         Some(Ok(ReturnValue::Throw(_))) => one,
            //         Some(Ok(ReturnValue::Abort)) => one,
            //         Some(Err(_)) => one,
            //         None => one
            //     };
            //     let zero = self.state.zero(32);  // The 32-bit constant 0
            //     if self.state.bvs_can_be_equal(&retval, &zero)? {
            //         return Err(Error::PathComplete);
            //     }
            }
            return retV;
    ...
    fn backtrack_and_continue(&mut self) -> Result<Option<ReturnValue<B::BV>>> {
        if self.state.revert_to_backtracking_point()? {
            ...
            //syncxxx-2-21, 避免因为其他error导致符号执行停止
            //Ok(None)
            Err(Error::OtherError(String::from("No Paths anymore!")))
        }
    }
        ...
        match self.state.type_of(icmp).as_ref() {
            Type::IntegerType { bits } if *bits == 1 => match op0_type.as_ref() {
                Type::IntegerType { .. } | Type::VectorType { .. } | Type::PointerType { .. } => {
                    //println!("DEBUG icmp: {:?}, {:?}, {:?}", bvpred(&bvfirstop, &bvsecondop),bvfirstop,bvsecondop);
    ...
    fn symex_load(&mut self, load: &'p instruction::Load) -> Result<()> {
        ...
        // println!("DEBUG: load addr:{:?}, {:?}",&bvaddr, self.state.read(&bvaddr, dest_size));
        //println!("DEBUG: load addr:{:?}",&bvaddr);
        self.state
    ...
    fn symex_store(&mut self, store: &'p instruction::Store) -> Result<()> {
        ...
        // println!("DEBUG: store addr:{:?}, {:?}",&bvaddr, bvval);
    ...
    fn symex_call(&mut self, call: &'p instruction::Call) -> Result<Option<ReturnValue<B::BV>>> {
        ...
        match self.resolve_function(&call.function)? {
            ...
            ResolvedFunction::NoHookActive { called_funcname } => {
                ...
                //syncxxx-1-11 不是有关键操作的函数不用进去
                let mut funcFlag = true;
                for func in &self.keyFuncs {
                    if func == called_funcname {
                        funcFlag = false;
                        break;
                    }
                }
                if let Some((callee, callee_mod)) =
                    self.state.get_func_by_name(called_funcname){
                    // match callee.get_debug_loc(){
                    //     Some(e) => {
                    //             let pretty_directory = match &e.directory {
                    //                 Some(dir) => dir,
                    //                 None => "",
                    //             };
                    //             let need_slash = match &e.directory {
                    //                 Some(dir) => !dir.is_empty() && !dir.ends_with('/') && !e.filename.starts_with('/'),
                    //                 None => false,
                    //             };
                    //             let pretty_filename = match &e.filename as &str {
                    ...
                    //                 filename => &filename,
                    //             };
                    //             let s3 = pretty_directory.to_string() + pretty_filename;
                    //             if s3.contains("linux-gnu") {
                    //                 funcFlag = true;
                    //             }
                    ...
                    // syncxxx-4-18 haybale 不支持未确定参数个数的函数
                ...
                //     }
                // }
                if (called_funcname == "mosquitto__malloc"
                    || called_funcname == "mosquitto__calloc"
                    || called_funcname == "mosquitto__realloc"
                    || called_funcname == "strcmp" 
                    || called_funcname == "packet__read_byte"
                    || called_funcname == "packet__read_bytes"
                    || called_funcname == "packet__read_binary"
                    || called_funcname == "packet__read_string"
                    || called_funcname == "packet__read_uint16"
                    || called_funcname == "packet__read_uint32"
                    || called_funcname == "packet__read_varint"
                    || called_funcname == "github_0com_1VolantMQ_1vlapi_1mqttp.Publish.SetQoS"
                    || called_funcname == "github_0com_1VolantMQ_1vlapi_1mqttp.Publish.SetRetain"
                    || called_funcname == "github_0com_1VolantMQ_1vlapi_1mqttp.Publish.Retain"
                    || called_funcname == "github_0com_1VolantMQ_1vlapi_1mqttp.Publish.QoS"
                    || called_funcname == "github_0com_1VolantMQ_1vlapi_1mqttp.QosType.IsValid"
                    // || called_funcname == "strlen" 
                    )
                {
                    funcFlag = false;
                }
                if at_max_callstack_depth || funcFlag {
                    ...
                    match self.state.type_of(call).as_ref() {
                        ...
                        ty => {
                            ...
                            // syncxxx-3-8
                            let instr = match self.state.cur_loc.instr {
                                BBInstrIndex::Instr(i) => i.to_string(),
                                BBInstrIndex::Terminator => "terminator".to_string()
                            };
                            let bv = self.state.new_bv_with_name(
                                Name::from(format!("{}_retval_{}:{}", called_funcname, self.state.cur_loc.bb.name, instr)),
                                width,
                            )?;
                            self.state
                                .assign_bv_to_name(call.dest.as_ref().unwrap().clone(), bv)?;
    ...
    fn resolve_function(
        ...
        let funcname_or_hook: Either<&str, FunctionHook<B>> = match function {
            ...
            Either::Right(operand) => {
                // syncxxx-4.26
                match self.state.config.function_hooks.get_default_hook(){
                    Some(hook) => Either::Right(hook.clone()),
    ...
    fn symex_condbr(
        ...
        //println!("Symexing condbr {:?}", condbr);
        let bvcond = self.state.operand_to_bv(&condbr.condition)?;
        // syncxxx-2-9
        let mut true_feasible = false;
        let mut false_feasible = false;
        let mut true_branch_name = String::from(self.state.cur_loc.func.name.as_str());
        true_branch_name = true_branch_name + &String::from(":");
        true_branch_name = true_branch_name + &(self.state.cur_loc.bb.name.to_string());
        true_branch_name = true_branch_name + &String::from("-");

        true_branch_name = true_branch_name + &(condbr.true_dest.to_string());
        //println!("true_branch_name: {}", true_branch_name);
        let mut false_branch_name = String::from(self.state.cur_loc.func.name.as_str());
        false_branch_name = false_branch_name + &String::from(":");    
        false_branch_name = false_branch_name + &(self.state.cur_loc.bb.name.to_string());;
        false_branch_name = false_branch_name + &String::from("-");
        false_branch_name = false_branch_name + &(condbr.false_dest.to_string());
        let mut idx_true: i32 = -1;
        let mut idx_false: i32 = -1;
        for (index, value) in self.keyBBs.iter().enumerate() {
            if value.as_str() == true_branch_name.as_str() {
                idx_true = index as i32;
                break;
            }
        }
        for (index, value) in self.keyBBs.iter().enumerate() {
            if value.as_str() == false_branch_name.as_str() {
                idx_false = index as i32;
                break;
            }
        }
        ...
        let mut funcFlag = true;
        for func in &self.keyFuncs {
            if func == self.state.cur_loc.func.name.as_str() {
                funcFlag = false;
                break;
            }
        }
        ...
        if true_feasible && false_feasible {
            ...
            // println!("true_branch_name: {}", true_branch_name);
            // println!("false_branch_name: {}", false_branch_name);
            // println!("neither branch is feasible");
            self.backtrack_and_continue()
        }
    }
    ...
    fn symex_switch(
        ...
        // syncxxx-1-23 只对keyBBs分支进行求解
        let mut filterDests: Vec<(ConstantRef, Name)> = Vec::new();
        for d in &switch.dests {
            let mut dest_name = String::from(self.state.cur_loc.func.name.as_str());
            dest_name = dest_name + &String::from(":");
            dest_name = dest_name + &(self.state.cur_loc.bb.name.to_string());
            dest_name = dest_name + &String::from("-");
            dest_name = dest_name + &d.1.to_string();
            let mut idx: i32 = -1;
            for (index, value) in self.keyBBs.iter().enumerate() {
                if value.as_str() == dest_name.as_str() {
                    idx = index as i32;
                    break;
                }
            }
            if idx != -1 {
                let n = d.1.clone();
                //println!("{}", d.1.to_string());
                filterDests.push((d.0.clone(), n));
            }
        }
        let mut funcFlag = true;
        for func in &self.keyFuncs {
        ...
        let mut dests = filterDests
            .iter()
            .map(|(c, n)| self.state.const_to_bv(c).map(|c| (c, n)))
            .collect::<Result<Vec<(B::BV, &Name)>>>()?;
        // syncxxx-4-17 普通函数随便进入
        if funcFlag {
            dests = switch
            .dests
            .iter()
        ...
        if feasible_dests.is_empty() {
            ...
            dest_name = dest_name + &(self.state.cur_loc.bb.name.to_string());
            dest_name = dest_name + &String::from("-");
            dest_name = dest_name + &(switch.default_dest.to_string());
            let mut idx: i32 = -1;
            for (index, value) in self.keyBBs.iter().enumerate() {
                if value.as_str() == dest_name.as_str() {
                    idx = index as i32;
            ...
            if idx != -1 {             
                let default_dest_constraint = allDests
                    .iter()
    ...
    fn symex_invoke(
        ...
        //println!("Symexing invoke {:?}", invoke);
