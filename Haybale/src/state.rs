    ...
    B: 'p,
{
    ...
    pub fn sat_with_extra_constraints<'b>(
        ...
        let a = solver_utils::sat_with_extra_constraints(&self.solver, constraints);
        return a;
    }
    ...
    pub fn new_bv(&self, width: u32) -> B::BV {
        B::BV::new(self.solver.clone(), width, None)
    }
        ...
        match c {
            ...
            Constant::GlobalReference { name, ty } => {
                    ...
                    .find(|a| &a.name == name)
                {
                    ...
                    // Ok(BV::new(&self.solver.clone(), size, None))
                    //syncxxx-4.18
                    Ok(self.new_bv(self.proj.pointer_size_bits()))
    ...
    // syncxxx-3-8
    pub fn get_pointer_to_global(&self, name: &Name) -> Option<&B::BV> {
        self.global_allocations
            .get_global_allocation(name, self.cur_loc.module)
            .map(|ga| ga.get_addr())
    }
