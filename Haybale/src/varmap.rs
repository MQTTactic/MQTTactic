...
impl<V: BV> VarMap<V> {
    ...
    pub fn new(solver: V::SolverRef, max_versions_of_name: usize) -> Self {
        Self {
            ...
            max_version_num: max_versions_of_name, // because 0 is a version
        }
    }
    ...
    pub fn assign_bv_to_name(&mut self, funcname: String, name: Name, bv: V) -> Result<()> {
        let new_version_num = self
            ...
            .or_insert(1); // insert a 0 if it didn't exist in map
        if *new_version_num > self.max_version_num
            && !(funcname == "strcmp"
                || funcname == "packet__read_byte"
                || funcname == "packet__read_bytes"
                || funcname == "packet__read_binary"
                || funcname == "packet__read_string"
                || funcname == "packet__read_uint16"
                || funcname == "packet__read_uint32"
                || funcname == "packet__read_varint")
        {
            // syncxxx-3-7 测试loopBound对于同一函数多次调用
